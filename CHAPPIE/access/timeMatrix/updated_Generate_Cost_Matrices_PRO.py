...
##    product_version = "{0} {1}".format(install_info["ProductName"], install_info["Version"])
    ...
    # Cutoff converted to distance to do selection of destinations
    rate = 5.0  # Default walk speed it 5 km/hr)
    max_dist = ((rate*1000)/60)*cutoff  # meters
    max_dist = str(max_dist) + ' meters'
    ...
    for run in range(st+1, runs+1):
        ...
        # select a subset of points from each layer for this run
        ...
        message("Origin whereClause: " + o_whereClause)
        #message("Destination whereClause: " + d_whereClause)
        ...
        #message("Generating Matrix Table using '{}' travel mode".format(mode))
...
class Cost_Matrix (object):
    ...
    def getParameterInfo(self):
        ...
        cutoff = setParam("Limit (min)", "cutoff", "GPDouble", "Optional", "")
        ...
        params = [origin_in, destination_in, travel_modes, cutoff, outTbl, st]
        return params
    def isLicensed(self):
        return True
